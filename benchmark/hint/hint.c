...
int main(__attribute__((unused)) int argc, char* argv[]) {
    ...
    printf("RECT is %lu bytes\n", sizeof(RECT));

#ifdef DEBUG
    curv = stdout;
#else
    sprintf(filnm, "curve_"DSIZESTR);
    ...
    if((tscout < RUNTM) && (eflag == NOMEM))
        ...
        printf(
            "Precision is not sufficient for > %3.1lf second runs.\n",
            RUNTM);
